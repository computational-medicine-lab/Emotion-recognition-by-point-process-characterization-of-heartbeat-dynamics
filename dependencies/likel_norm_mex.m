function varargout = likel_norm_mex(varargin)
% [thetap,k,steps,lambda,loglikel] = likel_norm_mex(xn, wn, eta, thetap0, k0, xt, wt, max_steps);
%
% Estimate the Gaussian parameters by maximum likelihood.
%
% [thetap,k,steps,loglikel] = likel_norm_mex(xn, wn);
% [thetap,k,steps,loglikel] = likel_norm_mex(xn, wn, eta);
% [thetap,k,steps,loglikel] = likel_norm_mex(xn, wn, eta, max_steps);
%
...
%            of the history-dependent gaussian distribution for each observation
%    k is the scale parameter of the gaussian distribution (corresponding to
...
%
%
% [thetap,k,steps,lambda,loglikel] = likel_norm_mex(xn, wn, eta, thetap0, k0, xt, wt);
% [thetap,k,steps,lambda,loglikel] = likel_norm_mex(xn, wn, eta, thetap0, k0, xt, wt, max_steps);
%
...
if exist('likel_norm_mex', 'file') ~= 3
    curpath = pwd();
    fpath = fileparts(which('likel_norm_mex'));
    cd(fpath);
    compiler = 'gnu';
    arch = '32';
    ...
    switch upper(compiler)
        case 'GNU'
            mex(['LDFLAGS="\$LDFLAGS -Wl,-rpath=' fpath '"'], '-L.', ['-llikel_invnorm' arch], 'likel_norm_mex.c');
        case {'LCC', 'MICROSOFT'}
...
if exist('likel_norm_mex', 'file') == 3
    ...
    [varargout{:}] = likel_norm_mex(varargin{:});
end
